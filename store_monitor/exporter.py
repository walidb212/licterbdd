...
from .models import RunArtifacts, StoreRecord, StoreReviewRecord
...
def build_run_artifacts(base_output_dir: str) -> RunArtifacts:
    ...
    return RunArtifacts(
        run_id=run_id,
        run_dir=str(run_dir),
        stores_path=str(run_dir / "stores.jsonl"),
...
def export_run(artifacts: RunArtifacts, stores: list[StoreRecord], reviews: list[StoreReviewRecord]) -> None:
    _write_jsonl(artifacts.stores_path, [store.to_dict() for store in stores])
